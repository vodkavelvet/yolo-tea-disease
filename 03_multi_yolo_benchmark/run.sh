#!/bin/bash
# Helper script untuk push dan pantau notebook Multi-YOLO Benchmark di Kaggle
# Usage:
#   sh run.sh push
#   sh run.sh status
#   sh run.sh watch
...
KERNEL_ID="teukuradja/multi-yolo-tea-disease-benchmark"
KAGGLE_BIN="../venv/bin/kaggle"
OUTPUT_DIR="../04_outputs/multi_yolo_downloaded_output"
SAVED_DIR="../04_outputs/multi_yolo_latest"
POLL_INTERVAL=60
...
cmd_push() {
    echo -e "${BLUE}📤 Pushing notebook Multi-YOLO ke Kaggle...${NC}"
...
cmd_watch() {
    echo -e "${BLUE}👀 Memantau benchmark...${NC}"
    ...
    while true; do
        ...
        if echo "$status_output" | grep -q "COMPLETE"; then
            ...
            echo -e "${RED}❌ Benchmark ERROR. Jalankan: sh run.sh logs${NC}"
            return 1
        elif echo "$status_output" | grep -q "CANCEL"; then
            echo -e "${YELLOW}⚠️ Benchmark dibatalkan${NC}"
            return 1
        fi
